...
export default function ManageAppointments() {
  ...
  const [scheduleType, setScheduleType] = useState<'kham_moi' | 'dieu_tri'>('kham_moi');
  ...
  const kpis = {
    ...
    waiting: dailyAppointments.filter(a => a.trang_thai === 'cho_xac_nhan').length,
    completed: dailyAppointments.filter(a => a.trang_thai === 'hoan_thanh').length,
    cancelled: dailyAppointments.filter(a => a.trang_thai === 'da_huy' || a.trang_thai === 'khong_den').length,
  };
    ...
    <div className="space-y-6 max-w-full">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center pb-2 gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Quản lý và điều phối chuyên sâu các ca khám và trị liệu.</p>
        </div>
        ...
        <div className="flex items-center bg-white rounded-xl shadow-sm border border-slate-100 p-1 self-stretch md:self-auto justify-between">
          ...
          <div className="px-5 text-sm font-semibold text-slate-700 text-center min-w-[200px]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between">
          <span className="text-slate-500 text-xs font-semibold uppercase tracking-wider">Tổng số ca trực</span>
